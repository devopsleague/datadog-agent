...
func TestBasicProcessMessages(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		t.Run(tc.testName, func(t *testing.T) {
			...
			messages, totalProcs, totalContainers := createProcCtrMessages(procs, tc.containers, cfg, tc.maxSize, maxBatchBytes, sysInfo, int32(i), "nid", 0)
...
func TestContainerProcessChunking(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		t.Run(tc.testName, func(t *testing.T) {
			...
			messages, totalProcs, totalContainers := createProcCtrMessages(processes, ctrs, cfg, tc.maxSize, maxBatchBytes, sysInfo, int32(i), "nid", 0)
