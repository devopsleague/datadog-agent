...
type Client struct {
	...
	apmInjectionListeners []func(update map[string]state.ConfigAPMTracing)
}
...
func (c *Client) RegisterCWSDDUpdate(fn func(update map[string]state.ConfigAPMTracing)) {
	c.m.Lock()
	defer c.m.Unlock()
...
func (c *Client) RegisterAPMInjectionUpdate(fn func(update map[string]state.ConfigAPMTracing)) {
	c.m.Lock()
	defer c.m.Unlock()
